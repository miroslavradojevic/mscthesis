...
void extendedKalmanFilter5DOF::init(filterMode whichMode, T_EKF5DOF_PARAMS parameters){
	...
	useGPSForFiltering = parameters.useGPSForFiltering;
	useCOMPASSForYawMeas = parameters.useCOMPASSForYawMeas;
	...
	setW(); setF(); Predict(); // extendedKalmanFilter5DOF methods
		...
		switch(whichSensor)
		{
			...
			case COMPASS_SENSOR:
			{
				...
				{// no need to measure yaw here, fog will do it instead
					H[0][PITCH_INDEX]      = 1; 
					H[1][PITCH_RATE_INDEX] = 1;
					R[0][0] = pow(SDpitch,2);
					R[1][1] = pow(SDpitchRate,2);
