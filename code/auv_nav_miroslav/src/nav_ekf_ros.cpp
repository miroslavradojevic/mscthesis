	...
	{// assumed aperiodic
		...
		requestShutdown();
				...
				if (!getINIFloat(EKF5DOF_params.SDpitchRateModel, "SDpitchRateModel", "EKF"))
						return false;
				if (!getINIBool(useGPSForFiltering, "useGPSForFiltering", "EKF"))
						return false;			
...
fprintf(pLogParams,"SDnorth: %f \nSDeast: %f \nSDdepth: %f \nSDaltitude: %f \nSDu: %f \nSDv: %f \nSDw: %f \nSDyaw: %f \nSDpitch: %f \nSDyawRate: %f \nSDpitchRate: %f \nSDuModel: %f \nSDvModel: %f \nSDwModel: %f \nSDyawRateModel: %f \nSDpitchRateModel: %f \nPeriodEKF: %i \n", EKF5DOF_params.SDnorth,EKF5DOF_params.SDeast,EKF5DOF_params.SDdepth,EKF5DOF_params.SDaltitude,
	...
	m_rosInputGpsSub = m_rosHandle->subscribe<auv_msgs::GPS>("nav/gps",
 			1, &navigationRos::handleGpsMsg, this, hints); 			
...
void navigationRos::handleROSnavMsg(const auv_msgs::NavStsConstPtr & msg)
{
    	...
    	if(EKF5DOF.isFilteringPossible())
    	{
    		...
    		northLBL = msg->position.north;
    		eastLBL  = msg->position.east;
		...
		MeasurementVec[8] = pitchRate;     MeasurementStates[8] = PITCH_RATE;
    		...
    		if (EKF5DOF.ekfMode==EVERY_MESSAGE)//if((EKF5DOF.ekfMode==EVERY_MESSAGE) || (EKF5DOF.ekfMode==EVERY_DT))//
    		{
    			double64 elapsedPeriod;
    			uint32_t elapsedMicroSeconds;
    			elapsedMicroSeconds = m_timer_ekf_MES->ReadAndReset();
    			elapsedPeriod = elapsedMicroSeconds * 0.000001;
						...
						previousP     = EKF5DOF.getPCorrected();
			olibVector<double64> filteredValues(SYSTEM_STATE_LEN,0); // storage for filtered state
			sensorType sensorDevice = NULL_SENSOR;
			filteredValues = EKF5DOF.filterMessage(MeasurementVec, sensorDevice, elapsedPeriod); 
						olibMatrix<double64> filteredP(SYSTEM_STATE_LEN,SYSTEM_STATE_LEN,0);
						filteredP     = EKF5DOF.getPCorrected();
			storeFilteringResults(filteredValues, elapsedPeriod);
...
void navigationRos::handleROSlblMsg(const nav_msgs::OdometryConstPtr & msg)
{
		...
		*/
		...
		MeasurementVec[1] = eastLBL;          MeasurementStates[1] = EAST;
    		...
    		if (1)//if((EKF5DOF.ekfMode==EVERY_MESSAGE) || (EKF5DOF.ekfMode==EVERY_DT))//
    		{
    			double64 elapsedPeriod;
    			uint32_t elapsedMicroSeconds;
    			elapsedMicroSeconds = m_timer_ekf_MES->ReadAndReset();
	...
	*/	
		...
		//gotGpsFix = true;
		*/
		if(gotHeading  && firstDepthMsgReceived && gotLBL) {//&& gotGpsFix
		EKF5DOF.setFilteringPossible();
		}
	}
	
} // LBL Msg
void navigationRos::handleGpsMsg(const auv_msgs::GPSConstPtr & msg)
{
/*
	if(msg->fix_quality >= 1)
	{
			LLD.lat = msg->latitude;
			LLD.lat = osl_core::wrapPlusMinus90(LLD.lat);
			LLD.lon = msg->longitude;
			LLD.lon = osl_core::wrapPlusMinus180(LLD.lon);
			LLD.depth = depth;
    	if(globalCoordConverter.isInitialised())
    	{
    		//float prevNorthGps, prevEastGps;
    		//prevNorthGps  = northGPS;
    		//prevEastGps   = eastGPS;
    		osl_core::T_NED gpsNED;
    		gpsNED = globalCoordConverter.LLD2NED(LLD);
    		north = northGPS = gpsNED.n;
		east = eastGPS  = gpsNED.e;
		fprintf(pLogGps, "%f, %f, %f\n", LLD.lat, LLD.lon, depth);
		cout << "### GPS message ###" << " N: " << northGPS << " E: " << eastGPS << endl;
    		if(EKF5DOF.isFilteringPossible() && useGPSForFiltering)
    		{
		MeasurementVec.setSize(2, 0);//enable enough space
		MeasurementStates.setSize(2, NULL_STATE);//enable enough space 
    		sensorType sensorDevice = LBL_SENSOR;
    		MeasurementVec[0] = northGPS;         MeasurementStates[0] = NORTH;
		MeasurementVec[1] = eastGPS;          MeasurementStates[1] = EAST;
    			double64 elapsedPeriod;
    			uint32_t elapsedMicroSeconds;
    			elapsedMicroSeconds = m_timer_ekf_MES->ReadAndReset();
    			elapsedPeriod = elapsedMicroSeconds * 0.000001;
    	...
    	{// first gps msg
    		if(useGPSForFiltering){
    			
		globalCoordConverter.setLLDReference(LLD);
		cout << "origin  L/L: " << LLD.lat << "  " <<  LLD.lon << endl;
		osl_core::T_NED currentNEDPosition;
    		currentNEDPosition = globalCoordConverter.LLD2NED(LLD);
    		originNORTH = northGPS = currentNEDPosition.n;
		originEAST = eastGPS = currentNEDPosition.e;	
		cout << "origin  N/E: " << originNORTH << "  " <<  originEAST << endl; 
     		olibVector<double64> X(SYSTEM_STATE_LEN, 0);
    		X[NORTH_INDEX]     = northGPS;
    		X[EAST_INDEX]      = eastGPS;
		X[DEPTH_INDEX]     = depth;
		X[ALTITUDE_INDEX]  = altitude;
		X[SURGE_VEL_INDEX] = surgeVelocity;
		X[SWAY_VEL_INDEX]  = swayVelocity;
		...
		gotGpsFix = true;// to enable filtering	
	}	
	}
	}// if gps quality
...
void navigationRos::storeLogFile(double64 interval, olibVector<double64> previousState, olibMatrix<double64> previousP, olibVector<double64> Obser, olibVector<stateID> ObserStates, olibVector<double64> filteredState, olibMatrix<double64> filteredP) {
	...
	// time interval
	// store the old nav
	fprintf(pOldNav, "%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n",northNAV, eastNAV, depth, altitude, surgeVelocity,swayVelocity, heaveVelocity, 
...
int main(int argc, char * argv[]) {
	...
	//fprintf(pLogGps, "latitude,longitude,elevation,name\n");
