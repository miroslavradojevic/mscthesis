...
plot(Obs(:,2), Obs(:,1), 'bo','MarkerSize',10);
plot(OutputMessages(:,2), OutputMessages(:,1), 'g', 'LineWidth', 3);
xlabel('east [m]'); ylabel('north [m]');
legend('dead reckoning','lbl', 'EKF localisation');

if(EXPORT_IMAGES),
            set(gcf, 'Color', 'none');  
            export_fig sim-straight2.eps -eps -a1;
end
...
figure;
subplot(211);
plot(time, (Obs(:,8))*180/pi, 'kx', 'MarkerSize',3);  hold on; ylabel('yaw [deg]');grid on;
...
subplot(212);
plot(time, Obs(:,10), 'kx', 'MarkerSize',3); hold on; ylabel('yaw rate [rad/s]'); xlabel('time [s]');
...
if(EXPORT_IMAGES),
    set(gcf, 'Color', 'none');  
    export_fig yaw-straight2.eps -eps -a1;
end
