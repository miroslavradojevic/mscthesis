...
figure;
subplot(211);
plot(time, (Obs(:,8))*180/pi, 'kx');  hold on; grid on; 
ylabel('compass yaw [deg]'); xlabel('time [s]');
plot(time, OutputMessages(:,8) *180/pi, 'r');
legend('measured', 'filtered');
%title ('compass');

subplot(212);
plot(time, Obs(:,10)*180/pi, 'kx'); hold on; grid on;
ylabel('FOG yaw rate [deg/s]'); xlabel('time [s]');
...
if(EXPORT_IMAGES),
    set(gcf, 'color', 'none');  
    export_fig magnetic.eps -eps -a1;
end
